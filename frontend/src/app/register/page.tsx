...
const RegisterPage = () => {
  const { handleSocialSignIn, signUpWithEmail, loading: isLoading, error, user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!recaptchaToken) {
      console.error('reCAPTCHA token no disponible');
      return;
    }

    try {
      // Aquí podrías validar el token en el backend antes de proceder
      await signUpWithEmail(formData.email, formData.password, formData.fullName, formData.phoneNumber);
    } catch (error) {
        ...
        <div className="-mx-4 flex flex-wrap">
          <div className="w-full px-4">
            <div className="shadow-three mx-auto max-w-[500px] rounded bg-white px-6 py-10 dark:bg-dark sm:p-[60px]">
              ...
              {/* Apple Sign-In Warning - Clean and modern style */}
